...
public class Nesterov implements Optimizer {
    ...
    public Nesterov(Map<String, Double> params) {
        learningRate = params.getOrDefault(OptimizerArgType.LEARNING_RATE, 0.01);
        momentum = params.getOrDefault(OptimizerArgType.MOMENTUM, 0.9);
    }
    ...
    public Map<String, NdArray> update(Map<String, NdArray> param, Map<String, NdArray> grad) {
        ...
        for (String key : param.keySet()) {
            ...
            v.put(
                    key,
                    v.get(key).mul(momentum)
                            .sub(grad.get(key).mul(learningRate))
            );

            result.put(
                    key,
                    param.get(key)
                            .add(v.get(key).mul(momentum * momentum))
                            .sub(grad.get(key).mul(learningRate * (1 - momentum)))
            );
        }
        return result;
    }
